...
class EddyImgPostProcess:
    ...
    def load_heatmap(self, raw_tag):
        path = self.tag_handler.tag_to_img_fname(raw_tag)
        img = Image.open(path)
    ...
    def process_heatmaps(self, target_tag):
        ...
        path, maps, legends = self.load_heatmap(raw_tag)
        path_f, maps_f, legends_f = self.load_heatmap(raw_tag_f)
        ...
        merged.save(self.tag_handler.tag_to_img_fname(target_tag))
    ...
    def process_flux(self, target_tag):
        ...
        fixed.save(self.tag_handler.tag_to_img_fname(target_tag))
