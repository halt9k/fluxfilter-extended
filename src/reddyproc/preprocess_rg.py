...
def prepare_rg(cfg_rep: RepConfig):
    # generate theoretical solar radiation using specifically pysolar
    
    if cfg_rep.debug:       
        # always generate theoretical Rg for comparison    
        rg_col = "Rg_th_Py"
    else:    
        if not cfg_rep.ustar_rg_source == "Rg_th_Py":
            return
        rg_col = cfg_rep.ustar_rg_source
    
    csv_header = pd.read_csv(cfg_rep.input_file, header=None, nrows=2, sep=' ')
    csv_header.columns = csv_header.iloc[0]
    csv_header = csv_header.loc[1:1]
    
    data = pd.read_csv(cfg_rep.input_file, header=0, names=csv_header.columns, skiprows=1, sep=' ')
    
    datetimes = (pd.to_datetime({'year': data['Year'], 'month': 1, 'day': 1}, utc=True) 
                 - pd.to_timedelta(1, unit='d') + pd.to_timedelta(data['DoY'], unit='d') 
                 + pd.to_timedelta(data['Hour'] - cfg_rep.timezone, unit='h'))
    altitude_degs = solar.get_altitude_fast(cfg_rep.latitude, cfg_rep.longitude, datetimes)
    ...
    csv_header.to_csv(cfg_rep.input_file, sep=' ', header=True, index=False)
    data.to_csv(cfg_rep.input_file, sep=' ', index=False, header=False, mode='a')
